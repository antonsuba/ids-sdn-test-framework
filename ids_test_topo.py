...
class IDSTestFramework(Topo):
    ...
    def create_internal_network(self, mac_ip_set, package=internal_network):
        for importer, modname, ispkg in pkgutil.iter_modules(package.__path__):
            ...
            try:
                self.int_hosts, self.int_switches, self.main_node = topo_class(
                ).create_topo(self, mac_ip_set)
    ...
    def create_external_network(self, ext_mac_set, package=external_network):
        ...
        for importer, modname, ispkg in pkgutil.iter_modules(package.__path__):
            ...
            try:
                self.ext_hosts, self.ext_switches = topo_class().create_topo(
                    self, ext_mac_set, offset, self.main_node)
    ...
    def generate_ip_aliases(self, hosts):
        ...
        offset = len(self.int_switches)
        self.ext_topo_class().generate_ip_aliases(hosts, self.ext_mac_ip_dict,
                                                  offset)
    ...
    def configure_router(self, router):
        ...
        self.ext_topo_class().configure_router(router, self.ext_mac_ip_dict,
                                               offset)
